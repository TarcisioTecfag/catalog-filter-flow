...
import { X, Play, ChevronLeft, ChevronRight, Cog, Info, Wrench, FileText, Download, Upload, Video, Image as ImageIcon, Pencil } from "lucide-react";
...
const MachineDetailModal = ({ machine, moduleName, open, onClose, lineId, moduleId, machineIndex }: MachineDetailModalProps) => {
  const [activeImageIndex, setActiveImageIndex] = useState(0);
  const [activeTab, setActiveTab] = useState<"info" | "specs" | "usage">("info");
  const [isEditing, setIsEditing] = useState(false);
  const galleryRef = useRef<HTMLDivElement>(null);
  const { isAdmin } = useAdmin();
  const { updateMachine } = useIndustrialLines();
  ...
  const handleSaveMachine = (updated: LineMachine) => {
    if (lineId && moduleId && machineIndex !== undefined && machineIndex >= 0) {
      updateMachine(lineId, moduleId, machineIndex, updated);
    }
    setIsEditing(false);
    onClose();
  };
